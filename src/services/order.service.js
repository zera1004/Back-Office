...
class orderService {
    ...
    createOrder = async ({ userId, restaurantId, cartId, status, total_price }) => {
        try {
            ...
            // 2. 결제 생성
            const payment = await this.#repository.createPayment({ userId, restaurantId, total_price });
            ...
            // 4. 포인트 업데이트
            await this.#repository.updateUserPoints({ userId, points: remainingPoints });
    ...
    deleteOrder = async ({ orderId }) => {
        try {
            ...
            // 3. 포인트 복원
            await this.#repository.restoreUserPoints({ userId: order.userId, refundedAmount: payment.total_price });
    ...
    checkOrder = async ({ orderId }) => {
        try {
            ...
            throw new Error("배달 상황을 확인하는 중 오류가 발생 담당자한테 문의할것.");
        }
    }
}
